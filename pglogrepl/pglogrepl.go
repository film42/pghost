...
func StartReplication(ctx context.Context, conn *pgconn.PgConn, slotName string, startLSN LSN, options StartReplicationOptions, args string) error {
	var timelineString string
	if options.Timeline > 0 {
		timelineString = fmt.Sprintf("TIMELINE %d", options.Timeline)
	}
	sql := fmt.Sprintf("START_REPLICATION SLOT %s %s %s %s %s", slotName, options.Mode, startLSN, timelineString, args)
